...
export default function HomeScreen({ name, currency, expenses, goals, theme, onTheme, onCoach }: Props) {
  ...
  const daily = Math.max(0, dailyBudget - expenses.filter(e => e.date === today).reduce((s,e)=>s+e.amount,0));
  return <div className="h-full app-scroll overflow-y-auto px-5 pb-28 pt-8 fade-in">
    ...
    <div className="mt-3 space-y-3">{expenses.slice(0,4).map(e=><article key={e.id} className="card flex min-w-0 items-center justify-between gap-3 rounded-3xl p-4"><div className="min-w-0"><h3 className="truncate font-bold">{e.name}</h3><p className="truncate text-xs text-[var(--muted)]">{e.category} · {e.description}</p></div><b className="shrink-0">{money(e.amount,currency)}</b></article>)}</div>
